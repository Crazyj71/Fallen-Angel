...
public class HellBossScript : MonoBehaviour
{
    ...
    {
        zoomedOut = false;
        death = GameObject.Find("DeathAnimationBoss");
        ...
        initHealth = health;
        Wings = GameObject.Find("WingsPickup");
    ...
    void FixedUpdate()
    {
        ...
        {
            if (rb2d.transform.position.x - player.transform.position.x < 20f
                && rb2d.transform.position.x - player.transform.position.x > -20f)
            {
                Vector3 dir = player.transform.position - transform.position;
                dir = dir.normalized;
                if ((dir.x < 0 && movementDirection > 0) || (dir.x > 0 && movementDirection < 0)) ChangeDirection();
                if (rb2d.transform.position.x - player.transform.position.x < 8f
                    && rb2d.transform.position.x - player.transform.position.x > -8f)
                    attacking = true;
                else
                {
                    attacking = false;
